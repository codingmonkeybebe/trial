...
def calc_cashflows_and_irr(
    ...
    for date, payment in milestone_payments:
        if date in cashflows.index:
            cashflows.loc[date] += payment
        else:
            cashflows = pd.concat([pd.Series([payment], index=[date]), cashflows]).sort_index()
    ...
    for y in range(5, project_life_years + 1, 5):
        ...
        dd_month_end = dd_date + pd.offsets.MonthEnd(0)
        if dd_month_end in cashflows.index:
            dd_cost = dd_cost_start * ((1 + annual_escalation) ** y)
            cashflows.loc[dd_month_end] -= dd_cost
    cashflows.iloc[-1] += scrap_value
    irr_monthly = npf.irr(cashflows.values)
...
# Average Daily Net Cash Flow Per Year
annual_total_cashflows = cashflows.resample("Y").sum()
annual_avg_daily_net_cashflow = annual_total_cashflows / 365

st.write("### Average Daily Net Cash Flow Per Year (USD/day)")
st.dataframe(annual_avg_daily_net_cashflow.apply(lambda x: f"${x:,.2f}"))
...
st.write("### Annual Cash Flow (USD)")
st.bar_chart(annual_total_cashflows)
